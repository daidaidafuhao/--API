...
namespace WebAPI.Data
{
    public class DatabaseManager
    {
        ...
        public List<String> LoadComboBoxItems(string query)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection($"Data Source={DatabaseFilePath};Version=3;"))
            {
                ...
                using (var command = new SQLiteCommand(query, connection))
                {
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            items.Add(reader[0]?.ToString() ?? string.Empty);
                        }
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var employee = new Employee
                            {
                                ...
                                Level = reader.IsDBNull(6) ? null : reader.GetString(6),
                                LevelJobType = reader.IsDBNull(7) ? null : reader.GetString(7),
                                Position = reader.IsDBNull(8) ? null : reader.GetString(8),
                                UnitName = reader.IsDBNull(9) ? null : reader.GetString(9),
                                RuzhiDate = reader.IsDBNull(10) ? DateTime.MinValue : DateTime.Parse(reader.GetString(10)),
                                SchoolName = reader.IsDBNull(11) ? null : reader.GetString(11),
                                ZhuanYe = reader.IsDBNull(12) ? null : reader.GetString(12)
                            };
        ...
        public void InsertTrainingRecord(TrainingRecord trainingRecord)
        {
            try
            {
                using (var connection = new SQLiteConnection($"Data Source={DatabaseFilePath};Version=3;"))
                {
                    ...
                    // 计算序号
                    int serialNumber = GetNextSerialNumber(connection, Convert.ToString(Convert.ToString(trainingRecord.EmployeeId)));
                ...
                VALUES (@EmployeeId, @SerialNumber, @TrainingDate, @TrainingContent, @TrainingUnit, @TrainingLocation, @Assessment, @Cost, @Remarks)";
                    ...
                    using (var command = new SQLiteCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@EmployeeId", Convert.ToString(trainingRecord.EmployeeId));
                        command.Parameters.AddWithValue("@SerialNumber", serialNumber.ToString());
        ...
        public void InsertTrainingRecordOrUpdate(TrainingRecord trainingRecord, SQLiteConnection connection, SQLiteTransaction transaction)
        {
            ...
            WHERE EmployeeId = @EmployeeId
              ...
              AND Remarks = @Remarks";
                ...
                using (var checkCommand = new SQLiteCommand(checkQuery, connection, transaction))
                {
                    string cleanedEmployeeId = Convert.ToString(trainingRecord.EmployeeId).Trim(); // 去除前后空格
                    ...
                    {
                        // 如果记录不存在，则插入新记录
                        int serialNumber = GetNextSerialNumber(connection, Convert.ToString(trainingRecord.EmployeeId));
        ...
        {
            // 使用参数化查询来避免 SQL 注入风险
            using (var command = new SQLiteCommand("SELECT MAX(SerialNumber) FROM TrainingRecord WHERE EmployeeId = @EmployeeId", connection))
            {
                ...
                return (result == DBNull.Value ? 0 : Convert.ToInt32(result)) + 1;
            }
        }
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var trainingRecord = new TrainingRecord
                            {
                                ...
                                TrainingContent = reader["TrainingContent"].ToString(),
                                TrainingUnit = reader["TrainingUnit"].ToString(),
                                TrainingLocation = reader["TrainingLocation"].ToString(),
                                Assessment = reader.IsDBNull(reader.GetOrdinal("Assessment")) ? null : reader["Assessment"].ToString(),
                                Cost = reader.IsDBNull(reader.GetOrdinal("Cost")) ? 0 : Convert.ToDecimal(reader["Cost"]),
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var trainingRecord = new TrainingRecord
                            {
                                ...
                                Cost = reader.IsDBNull(reader.GetOrdinal("Cost")) ? 0 : Convert.ToDecimal(reader["Cost"]),
                                Remarks = reader.IsDBNull(reader.GetOrdinal("Remarks")) ? null : reader["Remarks"].ToString(),
                                EmployeeId = reader.IsDBNull(reader.GetOrdinal("EmployeeId")) ? "0" : reader["EmployeeId"].ToString()
                            };
                            trainingRecords.Add(trainingRecord);
                        }
                    }
        ...
        public void InsertEmployeePhotoByIDCard(string idCardNumber, byte[] imageBytes)
        {
            try
            {
                using (var connection = new SQLiteConnection($"Data Source={DatabaseFilePath};Version=3;"))
